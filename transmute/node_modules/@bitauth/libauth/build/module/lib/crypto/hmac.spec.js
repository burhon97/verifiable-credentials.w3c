...
import { binToHex, hexToBin, hmacSha256, hmacSha512, instantiateSha256, instantiateSha512, } from '../lib';
const sha256Promise = instantiateSha256();
const sha512Promise = instantiateSha512();
const vectors = async (t, vector) => {
    const sha256 = await sha256Promise;
    const sha512 = await sha512Promise;
    t.deepEqual(hmacSha256(sha256, hexToBin(vector.secret), hexToBin(vector.message)), hexToBin(vector.sha256));
    t.deepEqual(hmacSha512(sha512, hexToBin(vector.secret), hexToBin(vector.message)), hexToBin(vector.sha512));
};
...
testProp('[fast-check] [crypto] hmacSha256 is equivalent to Node.js native HMAC-SHA256', [fcUint8Array(1, 100), fcUint8Array(1, 100)], async (t, secret, message) => {
    const sha256 = await sha256Promise;
    t.deepEqual(binToHex(hmacSha256(sha256, secret, message)), createHmac('sha256', secret).update(message).digest('hex'));
});
testProp('[fast-check] [crypto] hmacSha512 is equivalent to Node.js native HMAC-SHA512', [fcUint8Array(0, 100), fcUint8Array(0, 100)], async (t, secret, message) => {
    const sha512 = await sha512Promise;
    t.deepEqual(binToHex(hmacSha512(sha512, secret, message)), createHmac('sha512', secret).update(message).digest('hex'));
});
