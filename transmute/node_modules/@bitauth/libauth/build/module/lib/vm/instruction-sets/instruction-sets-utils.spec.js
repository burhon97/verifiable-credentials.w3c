...
import { assembleBytecode, assembleBytecodeBCH, assembleBytecodeBTC, authenticationInstructionsAreMalformed, disassembleBytecode, disassembleBytecodeBCH, disassembleBytecodeBTC, disassembleParsedAuthenticationInstructions, generateBytecodeMap, hexToBin, OpcodesBCH, OpcodesBTC, parseBytecode, range, serializeAuthenticationInstruction, serializeAuthenticationInstructions, serializeParsedAuthenticationInstructions, } from '../../lib';
...
const disassemble = (t, input, expected) => {
    t.deepEqual(disassembleParsedAuthenticationInstructions(OpcodesBCH, input), expected);
};
...
testProp('[fast-check] disassembleBytecodeBCH <-> assembleBytecodeBCH', [fcUint8Array(0, maxBinLength)], (t, randomBytecode) => {
    const parsed = parseBytecode(randomBytecode);
    const instructions = (authenticationInstructionsAreMalformed(parsed)
        ? parsed.slice(0, -1)
        : parsed);
    const minimalPush = instructions.map((instruction) => [OpcodesBCH.OP_PUSHDATA_2, OpcodesBCH.OP_PUSHDATA_4].includes(instruction.opcode)
...
testProp('[fast-check] disassembleBytecodeBTC <-> assembleBytecodeBTC', [fcUint8Array(0, maxBinLength)], (t, randomBytecode) => {
    const parsed = parseBytecode(randomBytecode);
    const instructions = (authenticationInstructionsAreMalformed(parsed)
        ? parsed.slice(0, -1)
        : parsed);
