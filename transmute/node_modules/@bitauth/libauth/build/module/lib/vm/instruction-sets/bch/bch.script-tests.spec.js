...
pendingTests.map((expectation) => {
    const description = `[script_tests] ${expectation.testIndex}/${pendingTests.length} – "${elide(expectation.unlockingBytecodeText, 100)}" | "${elide(expectation.lockingBytecodeText, 100)}"${expectation.message === undefined ? '' : ` # ${expectation.message}`}`;
