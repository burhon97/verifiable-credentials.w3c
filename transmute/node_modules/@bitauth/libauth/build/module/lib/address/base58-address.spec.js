...
import { Base58AddressError, Base58AddressFormatVersion, decodeBase58Address, decodeBase58AddressFormat, decodePrivateKeyWif, encodeBase58Address, encodeBase58AddressFormat, hexToBin, instantiateSha256, lockingBytecodeToBase58Address, } from '../lib';
...
test('encodeBase58AddressFormat', async (t) => {
    ...
    t.deepEqual(encodeBase58AddressFormat(sha256, Base58AddressFormatVersion.p2pkh, payload), 
...
test('encodeBase58Address', async (t) => {
    ...
    t.deepEqual(encodeBase58Address(sha256, 'p2pkh', payload), 
    // cspell: disable-next-line
    '1BpEi6DfDAUFd7GtittLSdBeYJvcoaVggu');
    t.deepEqual(encodeBase58Address(sha256, 'p2pkh-testnet', payload), 
    // cspell: disable-next-line
    'mrLC19Je2BuWQDkWSTriGYPyQJXKkkBmCx');
    t.deepEqual(encodeBase58Address(sha256, 'p2pkh-copay-bch', payload), 
    // cspell: disable-next-line
    'CTH8H8Zj6DSnXFBKQeDG28ogAS92iS16Bp');
    t.deepEqual(encodeBase58Address(sha256, 'p2sh', payload), 
    // cspell: disable-next-line
    '3CWFddi6m4ndiGyKqzYvsFYagqDLPVMTzC');
    t.deepEqual(encodeBase58Address(sha256, 'p2sh-testnet', payload), 
    // cspell: disable-next-line
    '2N44ThNe8NXHyv4bsX8AoVCXquBRW94Ls7W');
    t.deepEqual(encodeBase58Address(sha256, 'p2sh-copay-bch', payload), 
    // cspell: disable-next-line
...
testProp('[fast-check] encodeBase58Address <-> decodeBase58Address', [fc.integer(0, maxUint8Number), fcUint8Array(0, maxBinLength)], async (t, version, payload) => {
    const sha256 = await sha256Promise;
...
test('decodeBase58AddressFormat: errors', async (t) => {
    ...
    t.deepEqual(
    // cspell: disable-next-line
    decodeBase58AddressFormat(sha256, '1AGNa15ZQXAZUgFiqJ2i7Z2DPU2J6hW62a'), Base58AddressError.invalidChecksum);
    t.deepEqual(decodeBase58AddressFormat(sha256, '1234'), Base58AddressError.tooShort);
    t.deepEqual(
    // cspell: disable-next-line
    decodeBase58AddressFormat(sha256, '1AGNa15ZQXAZUgFiqJ2i7Z2DPU2J6hW62I'), Base58AddressError.unknownCharacter);
});
test('decodeBase58Address: errors', async (t) => {
    const sha256 = await sha256Promise;
    t.deepEqual(
    // cspell: disable-next-line
    decodeBase58Address(sha256, '6PfDNQxJdsBx7K4r9kMrRBZSa2NZKVNUZn'), Base58AddressError.unknownAddressVersion);
    t.deepEqual(
    // cspell: disable-next-line
...
test('Base58Address Valid Vectors (from C++ implementation – includes WIF vectors)', async (t) => {
    ...
    validVectors.forEach((vectors) => {
        ...
        if (meta.isPrivkey) {
            ...
            t.deepEqual(lockingBytecodeToBase58Address(sha256, hexToBin(lockingBytecode), testnet ? 'testnet' : 'mainnet'), base58Address);
        }
