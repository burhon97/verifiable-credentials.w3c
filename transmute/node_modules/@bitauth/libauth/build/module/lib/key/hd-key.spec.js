...
test('[crypto] deriveHdPrivateNodeIdentifier', async (t) => {
    const crypto = await instantiateBIP32Crypto();
    const { node } = decodeHdPrivateKey(crypto, xprv);
    t.deepEqual(deriveHdPrivateNodeIdentifier(crypto, node), hexToBin('15c918d389673c6cd0660050f268a843361e1111'));
});
test('[crypto] deriveHdPublicNodeIdentifier', async (t) => {
    const crypto = await instantiateBIP32Crypto();
    const { node } = decodeHdPublicKey(crypto, xpub);
    t.deepEqual(deriveHdPublicNodeIdentifier(crypto, node), hexToBin('15c918d389673c6cd0660050f268a843361e1111'));
});
...
test('[crypto] decodeHdKey: errors', async (t) => {
    const crypto = await instantiateBIP32Crypto();
    t.deepEqual(decodeHdKey(crypto, '#badKey'), HdKeyDecodingError.unknownCharacter);
    t.deepEqual(decodeHdKey(crypto, 'xprv1234'), HdKeyDecodingError.incorrectLength);
    t.deepEqual(decodeHdKey(crypto, 'xpub661MyMwAqRbcEngHLkoUWCKMBMGeqdqCGkqtzrHaTZub9ALw2oRfHA6fQP5n5X9VHStaNTBYomkSb8BFhUGavwD3RG1qvMkEKceTavTp2Ta'), HdKeyDecodingError.invalidChecksum);
});
...
test('[crypto] decodeHdPrivateKey: errors', async (t) => {
    const crypto = await instantiateBIP32Crypto();
    t.deepEqual(decodeHdPrivateKey(crypto, xpub), HdKeyDecodingError.privateKeyExpected);
    t.deepEqual(decodeHdPrivateKey(crypto, '1111111111111FF9QeH94hg7KAjgjUqkHUqbrw5wWQLoRNfRhB4cHUDCJxx2HfNb5qDiAjpbKjXeLJSknuzDmja42174H9Es1XbY24sZts9'), HdKeyDecodingError.unknownVersion);
    ...
    t.deepEqual(decodeHdPrivateKey(crypto, xprvWith0FilledKey), HdKeyDecodingError.invalidPrivateNode);
    ...
    t.deepEqual(decodeHdPrivateKey(crypto, xprvWith255FilledKey), HdKeyDecodingError.invalidPrivateNode);
    t.deepEqual(decodeHdPrivateKey(crypto, 'xprv9s21ZrQH143K2JbpEjGU94NcdKSASB7LuXvJCTsxuENcGN1nVG7QjMnBhegPMjkj1oGSFcmBkMX3xdwcMy6NSgrHvmqJptpUW5xGjg7kifZ'), HdKeyDecodingError.missingPrivateKeyPaddingByte);
});
...
test('[crypto] decodeHdPublicKey: errors', async (t) => {
    const crypto = await instantiateBIP32Crypto();
    t.deepEqual(decodeHdPublicKey(crypto, xprv), HdKeyDecodingError.publicKeyExpected);
    t.deepEqual(decodeHdPublicKey(crypto, '1111111111111FF9QeH94hg7KAjgjUqkHUqbrw5wWQLoRNfRhB4cHUDCJxx2HfNb5qDiAjpbKjXeLJSknuzDmja42174H9Es1XbY24sZts9'), HdKeyDecodingError.unknownVersion);
});
test('[crypto] encodeHdPrivateKey', async (t) => {
    const crypto = await instantiateBIP32Crypto();
    t.deepEqual(encodeHdPrivateKey(crypto, {
        network: 'mainnet',
        node: {
            chainCode: hexToBin('18aab7e9ef169f3029d93651d0c85303cbcc2ac559ccd04c324a2e678ef26dc9'),
            childIndex: 0,
...
test('[crypto] encodeHdPublicKey', async (t) => {
    const crypto = await instantiateBIP32Crypto();
    t.deepEqual(encodeHdPublicKey(crypto, {
        network: 'mainnet',
        node: {
            chainCode: hexToBin('18aab7e9ef169f3029d93651d0c85303cbcc2ac559ccd04c324a2e678ef26dc9'),
            childIndex: 0,
...
test('[crypto] deriveHdPrivateNodeChild', async (t) => {
    ...
    const hardenedIndex0Child = decodeHdPrivateKey(crypto, 'xprv9u4S6TaiPQaF7FS24QFpRP6hjff4jXNwwYTkVNC6f6YzHj2a6G28aRN1D6Az82SxMeBivpVS3gDDXyQiu3RANTqWy34Zxi9JN76zSwkjqPF');
    const index1GrandChild = decodeHdPrivateKey(crypto, 'xprv9w8PdihBAeR4xgGYWWqBnmDTrpWEW1QjuYAUkR7A6X48q1iQVgN433aSFxQGgtureVz7cCyi5zfuMTtBF3AkanjtvNs9m8u2JobxNfphSi3');
    ...
    const result0 = deriveHdPrivateNodeChild(crypto, master.node, hardenedIndex0);
...
test('[crypto] deriveHdPrivateNodeChild: errors', async (t) => {
    ...
    t.deepEqual(deriveHdPrivateNodeChild(crypto, node, max + 1), HdNodeDerivationError.childIndexExceedsMaximum);
});
test('[crypto] deriveHdPublicNodeChild', async (t) => {
    ...
    const child = deriveHdPrivateNodeChild(crypto, node, derivationIndex);
    const expectedPublic = deriveHdPublicNode(crypto, child);
    t.deepEqual(deriveHdPublicNodeChild(crypto, parentPublic, derivationIndex), expectedPublic);
});
test('[crypto] deriveHdPublicNodeChild: errors', async (t) => {
    const crypto = await instantiateBIP32Crypto();
    const { node } = decodeHdPublicKey(crypto, xpub);
    const hardened0 = 0x80000000;
    t.deepEqual(deriveHdPublicNodeChild(crypto, node, hardened0), HdNodeDerivationError.hardenedDerivationRequiresPrivateNode);
});
test('[crypto] deriveHdPath', async (t) => {
    const crypto = await instantiateBIP32Crypto();
    const { node: privateNode } = decodeHdPrivateKey(crypto, xprv);
    const publicNode = deriveHdPublicNode(crypto, privateNode);
    t.deepEqual(deriveHdPath(crypto, privateNode, 'm'), privateNode);
    t.deepEqual(deriveHdPath(crypto, publicNode, 'M'), publicNode);
    t.deepEqual(deriveHdPath(crypto, privateNode, "m/0'/1"), {
        ...decodeHdPrivateKey(crypto, 'xprv9w8PdihBAeR4xgGYWWqBnmDTrpWEW1QjuYAUkR7A6X48q1iQVgN433aSFxQGgtureVz7cCyi5zfuMTtBF3AkanjtvNs9m8u2JobxNfphSi3').node,
        parentIdentifier: hexToBin('2f2bc501c943dd7f17904b612c090dd88270cc59'),
    });
    t.deepEqual(deriveHdPath(crypto, publicNode, 'M/0/1/2/3'), deriveHdPublicNode(crypto, deriveHdPath(crypto, privateNode, 'm/0/1/2/3')));
    t.deepEqual(deriveHdPath(crypto, privateNode, "m/0'/1'/2'/3'"), deriveHdPath(crypto, privateNode, 'm/2147483648/2147483649/2147483650/2147483651'));
});
test('[crypto] deriveHdPath: errors', async (t) => {
    const crypto = await instantiateBIP32Crypto();
    const { node: privateNode } = decodeHdPrivateKey(crypto, xprv);
    const publicNode = deriveHdPublicNode(crypto, privateNode);
    t.deepEqual(deriveHdPath(crypto, privateNode, 'm/bad/1'), HdNodeDerivationError.invalidDerivationPath);
    t.deepEqual(deriveHdPath(crypto, privateNode, 'M'), HdNodeDerivationError.invalidPrivateDerivationPrefix);
    t.deepEqual(deriveHdPath(crypto, publicNode, 'm'), HdNodeDerivationError.invalidPublicDerivationPrefix);
    t.deepEqual(deriveHdPath(crypto, privateNode, 'm/0/4294967296/0'), HdNodeDerivationError.childIndexExceedsMaximum);
...
test('[crypto] crackHdPrivateNodeFromHdPublicNodeAndChildPrivateNode', async (t) => {
    const crypto = await instantiateBIP32Crypto();
    const { node: parentPrivateNode } = decodeHdPrivateKey(crypto, xprv);
    const parentPublicNode = deriveHdPublicNode(crypto, parentPrivateNode);
    const nonHardenedChildNode = deriveHdPath(crypto, parentPrivateNode, 'm/1234');
    const hardenedChildNode = deriveHdPath(crypto, parentPrivateNode, "m/1234'");
    const hardenedChildPublicNode = deriveHdPublicNode(crypto, hardenedChildNode);
    const nonHardenedGrandchildNode = deriveHdPath(crypto, hardenedChildNode, 'm/1234');
    t.deepEqual(crackHdPrivateNodeFromHdPublicNodeAndChildPrivateNode(crypto, parentPublicNode, nonHardenedChildNode), parentPrivateNode);
    t.deepEqual(crackHdPrivateNodeFromHdPublicNodeAndChildPrivateNode(crypto, hardenedChildPublicNode, nonHardenedGrandchildNode), hardenedChildNode);
    t.deepEqual(crackHdPrivateNodeFromHdPublicNodeAndChildPrivateNode(crypto, parentPublicNode, hardenedChildNode), HdNodeCrackingError.cannotCrackHardenedDerivation);
});
...
const fcBip32Path = () => fc
    ...
    .join('/')}`);
testProp('[fast-check] [crypto] HD key derivation is equivalent to bitcore-lib-cash', [fcBip32Path()], async (t, path) => {
    const crypto = await instantiateBIP32Crypto();
    const privateNode = decodeHdPrivateKey(crypto, xprv).node;
    const node = deriveHdPath(crypto, privateNode, path);
...
// eslint-disable-next-line max-params
    ...
    t.deepEqual(encoded, encodeHdPublicKey(crypto, decodeHdPublicKey(crypto, encoded)));
});
...
// eslint-disable-next-line max-params
    ...
    t.deepEqual(encoded, encodeHdPrivateKey(crypto, decodeHdPrivateKey(crypto, encoded)));
});
...
// eslint-disable-next-line max-params
    ...
    const { node: parentPrivateNode } = decodeHdPrivateKey(crypto, parentXprv);
    const parentPublicNode = deriveHdPublicNode(crypto, parentPrivateNode);
    const nonHardenedChildNode = deriveHdPrivateNodeChild(crypto, parentPrivateNode, childIndexes);
    const crackedParentNode = crackHdPrivateNodeFromHdPublicNodeAndChildPrivateNode(crypto, parentPublicNode, nonHardenedChildNode);
