...
import { authenticationTemplateP2pkh, authenticationTemplateP2pkhNonHd, authenticationTemplateToCompilationEnvironment, authenticationTemplateToCompilerBCH, compilerOperationsBCH, createAuthenticationProgramEvaluationCommon, createCompiler, extendedScenarioDefinitionToCompilationData, extendScenarioDefinition, extendScenarioDefinitionData, generateBytecodeMap, generateDefaultScenarioDefinition, generateExtendedScenario, hexToBin, instantiateRipemd160, instantiateSecp256k1, instantiateSha256, instantiateSha512, instantiateVirtualMachineBCH, instructionSetBCHCurrentStrict, OpcodesBCH, stringifyTestVector, validateAuthenticationTemplate, } from '../lib';
...
test('generateDefaultScenarioDefinition: missing sha256', async (t) => {
    ...
    t.deepEqual(scenario, 'An implementations of "sha256" is required to generate defaults for HD keys, but the "sha256" property is not included in this compilation environment.', stringifyTestVector(scenario));
});
test('generateDefaultScenarioDefinition: missing sha512', async (t) => {
    ...
    t.deepEqual(scenario, 'An implementations of "sha512" is required to generate defaults for HD keys, but the "sha512" property is not included in this compilation environment.', stringifyTestVector(scenario));
});
...
test('generateExtendedScenario: unknown scenario identifier', (t) => {
    ...
    t.deepEqual(extended, 'Cannot extend scenario "unknown": a scenario with the identifier unknown is not included in this compilation environment.', stringifyTestVector(extended));
});
...
test('generateDefaultScenarioDefinition: authenticationTemplateP2pkhNonHd', (t) => {
    const environment = authenticationTemplateToCompilationEnvironment(authenticationTemplateP2pkhNonHd);
...
test('generateDefaultScenarioDefinition: authenticationTemplateP2pkh', async (t) => {
    ...
    const sha512 = await sha512Promise;
    const environment = {
        ...authenticationTemplateToCompilationEnvironment(authenticationTemplateP2pkh),
        sha256,
        sha512,
    };
    const scenario = generateDefaultScenarioDefinition(environment);
 ...
 */
export const expectScenarioGenerationResult = async (t, scenarioId, unlockingScriptId, templateOverrides, expectedResult, environmentOverrides
...
test('generateScenario: no scenarios', expectScenarioGenerationResult, 'does_not_exist', 'unlock', { scenarios: undefined }, 'Cannot generate scenario "does_not_exist": a scenario with the identifier does_not_exist is not included in this compilation environment.');
...
test('generateScenario: no scenario ID', expectScenarioGenerationResult, undefined, 'unlock', {
    scenarios: {
        a: {},
    },
}, {
...
test('generateScenario: unknown locking bytecode script', expectScenarioGenerationResult, 'a', 'unlock', {
    scenarios: {
...
test('generateScenario: no locking script, no specified unlocking script', expectScenarioGenerationResult, 'a', undefined, {
    scenarios: {
        a: {
            transaction: { outputs: [{ lockingBytecode: {} }] },
        },
...
test('generateScenario: simple transaction, locking bytecode override', expectScenarioGenerationResult, 'a', 'unlock', {
    scenarios: {
        a: {
            data: {
...
test('generateScenario: complex transaction, locking bytecode variable override', expectScenarioGenerationResult, 'a', 'unlock', {
    scenarios: {
        a: {
...
test('generateScenario: locking bytecode generation failure', expectScenarioGenerationResult, 'a', 'unlock', {
    scenarios: {
        a: {
            transaction: {
                outputs: [
