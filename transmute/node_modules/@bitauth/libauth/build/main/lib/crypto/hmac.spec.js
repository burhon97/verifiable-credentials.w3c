...
const vectors = async (t, vector) => {
    const sha256 = await sha256Promise;
    const sha512 = await sha512Promise;
    t.deepEqual(lib_1.hmacSha256(sha256, lib_1.hexToBin(vector.secret), lib_1.hexToBin(vector.message)), lib_1.hexToBin(vector.sha256));
    t.deepEqual(lib_1.hmacSha512(sha512, lib_1.hexToBin(vector.secret), lib_1.hexToBin(vector.message)), lib_1.hexToBin(vector.sha512));
};
// eslint-disable-next-line functional/immutable-data
vectors.title = (title) => `[crypto] HMAC Test Vector #${title !== null && title !== void 0 ? title : '?'} (RFC 4231)`;
ava_1.default('1', vectors, {
...
ava_1.default('3', vectors, {
    message: '7768617420646f2079612077616e7420666f72206e6f7468696e673f',
    secret: '4a656665',
    sha256: '5bdcc146bf60754e6a042426089575c75a003f089d2739839dec58b964ec3843',
    sha512: '164b7a7bfcf819e2e395fbe73b56e0a387bd64222e831fd610270cd7ea2505549758bf75c05a994a6d034f65f8f0e6fdcaeab1a34d4a6b4b636e070a38bce737',
});
ava_1.default('4', vectors, {
...
const fcUint8Array = (minLength, maxLength) => ava_fast_check_1.fc
    .array(ava_fast_check_1.fc.integer(0, 255), minLength, maxLength)
    .map((a) => Uint8Array.from(a));
ava_fast_check_1.testProp('[fast-check] [crypto] hmacSha256 is equivalent to Node.js native HMAC-SHA256', [fcUint8Array(1, 100), fcUint8Array(1, 100)], async (t, secret, message) => {
    const sha256 = await sha256Promise;
    t.deepEqual(lib_1.binToHex(lib_1.hmacSha256(sha256, secret, message)), crypto_1.createHmac('sha256', secret).update(message).digest('hex'));
});
ava_fast_check_1.testProp('[fast-check] [crypto] hmacSha512 is equivalent to Node.js native HMAC-SHA512', [fcUint8Array(0, 100), fcUint8Array(0, 100)], async (t, secret, message) => {
    const sha512 = await sha512Promise;
    t.deepEqual(lib_1.binToHex(lib_1.hmacSha512(sha512, secret, message)), crypto_1.createHmac('sha512', secret).update(message).digest('hex'));
});
