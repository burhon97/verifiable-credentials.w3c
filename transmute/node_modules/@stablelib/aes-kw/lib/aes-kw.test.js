...
var keyEncryptionKey = "000102030405060708090A0B0C0D0E0F";
var tooShortKeyData = "001122334455";
describe("wrapKey", function () {
    testVectors.forEach(function (v) {
        it("should wrap " + v.Description, function () {
            var aeskw = new aes_kw_1.AESKW(hex_1.decode(v.KEK));
            var wrappedKey = aeskw.wrapKey(hex_1.decode(v.KeyData));
            expect(hex_1.encode(wrappedKey)).toBe(v.WrappedKey);
        });
    });
    it("should throw an error wrapping keyData that does not have sufficient length", function () {
        var aeskw = new aes_kw_1.AESKW(hex_1.decode(keyEncryptionKey));
        expect(function () { return aeskw.wrapKey(hex_1.decode(tooShortKeyData)); })
            .toThrowError(/16 bytes/);
    });
});
describe("unWrapKey", function () {
    testVectors.forEach(function (v) {
        it("should unWrap " + v.Description, function () {
            var aeskw = new aes_kw_1.AESKW(hex_1.decode(v.KEK));
            var keyData = aeskw.unwrapKey(hex_1.decode(v.WrappedKey));
            expect(hex_1.encode(keyData)).toBe(v.KeyData);
        });
    });
    it("should throw an error un-wrapping a wrapped key that does not have sufficient length", function () {
        var aeskw = new aes_kw_1.AESKW(hex_1.decode(keyEncryptionKey));
        expect(function () { return aeskw.unwrapKey(hex_1.decode(tooShortKeyData)); })
            .toThrowError(/16 bytes/);
    });
    it("should throw an error unWrapping a wrapped key that does not contain the default IV", function () {
        var aeskw = new aes_kw_1.AESKW(hex_1.decode(keyEncryptionKey));
        expect(function () { return aeskw.unwrapKey(hex_1.decode("E2EF32C0BBC36B6D236C2FD28E68FCDA8E24204C59F98754889D1ED24364A4D130AF535D350B9243")); })
            .toThrowError(/integrity/);
    });
});
//# sourceMappingURL=aes-kw.test.js.map